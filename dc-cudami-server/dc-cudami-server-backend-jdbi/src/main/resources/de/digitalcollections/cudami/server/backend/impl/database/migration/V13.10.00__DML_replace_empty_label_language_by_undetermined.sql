...
CREATE OR REPLACE FUNCTION jsonarray_replace_language (jsonb) RETURNS jsonb
RETURNS NULL ON NULL INPUT
IMMUTABLE
PARALLEL SAFE
LANGUAGE plpgsql AS
...
BEGIN
  ...
  ASSERT jsonarray_replace_language('[{"": "some text"}, {"en__#Latn": "something", "de": "text"}]')
...
UPDATE entities SET notes = jsonarray_replace_language(notes) WHERE jsonb_path_exists(notes, '$[*].keyvalue()[*] ? (@.key == "" || @.key like_regex "[_#]")');
