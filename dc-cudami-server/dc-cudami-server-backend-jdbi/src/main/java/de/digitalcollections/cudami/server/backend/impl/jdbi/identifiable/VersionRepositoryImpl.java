...
public class VersionRepositoryImpl implements VersionRepository {
  ...
  public Version findOne(UUID uuid) {
    Version version = (Version) dbi.withHandle(h -> h.createQuery(
      "SELECT * FROM versions WHERE uuid = :uuid")
      .bind("uuid", uuid)
  ...
  public Version findOneByInstanceversionKey(String instVersionKey) {
    Optional<VersionImpl> version = (Optional<VersionImpl>) dbi.withHandle(h -> h.createQuery(
  ...
  public Version save(Version version) {
    ...
    VersionImpl result = dbi.withHandle(h -> h
      .createQuery("INSERT INTO versions(uuid, version_value, type_key, instance_key, instance_version_key, description, created, status) "
  ...
  public Version update(Version version) {
    ...
    Version result = dbi.withHandle(h -> h
      .createQuery("UPDATE versions SET status=:status WHERE uuid=:uuid RETURNING *")
      .bindBean(version)
      .mapToBean(VersionImpl.class)
      .findOne().orElse(null));
