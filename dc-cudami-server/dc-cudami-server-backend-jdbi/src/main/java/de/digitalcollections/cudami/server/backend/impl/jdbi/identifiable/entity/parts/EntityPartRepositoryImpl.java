...
public class EntityPartRepositoryImpl<P extends EntityPart, E extends Entity> extends IdentifiableRepositoryImpl<P> implements EntityPartRepository<P, E> {
  ...
  public void addRelatedEntity(UUID entityPartUuid, UUID entityUuid) {
    Integer sortIndex = selectNextSortIndexForParentChildren(dbi, "rel_entitypart_entities", "entitypart_uuid", entityPartUuid);
    dbi.withHandle(h -> h.createUpdate("INSERT INTO rel_entitypart_entities(entitypart_uuid, entity_uuid, sortindex) VALUES (:entitypart_uuid, :entity_uuid, :sortindex)")
  ...
  public void addRelatedFileresource(UUID entityPartUuid, UUID fileResourceUuid) {
    Integer sortIndex = selectNextSortIndexForParentChildren(dbi, "rel_entitypart_fileresources", "entitypart_uuid", entityPartUuid);
    dbi.withHandle(h -> h.createUpdate("INSERT INTO rel_entitypart_fileresources(entitypart_uuid, fileresource_uuid, sortindex) VALUES (:entitypart_uuid, :fileresource_uuid, :sortindex)")
      .bind("entitypart_uuid", entityPartUuid)
  ...
  public LinkedHashSet<E> getRelatedEntities(UUID entityPartUuid) {
    StringBuilder query = new StringBuilder("SELECT *")
      .append(" FROM entities e INNER JOIN rel_entitypart_entities ref ON e.uuid=ref.entity_uuid")
      .append(" WHERE ref.entitypart_uuid = :entityPartUuid")
      .append(" ORDER BY ref.sortindex");

    List<EntityImpl> list = dbi.withHandle(h -> h.createQuery(query.toString())
  ...
  public LinkedHashSet<FileResource> getRelatedFileResources(UUID entityPartUuid) {
    StringBuilder query = new StringBuilder("SELECT *")
      .append(" FROM fileresources f INNER JOIN rel_entitypart_fileresources ref ON f.uuid=ref.fileresource_uuid")
      .append(" WHERE ref.entitypart_uuid = :entityPartUuid")
      .append(" ORDER BY ref.sortindex");
  ...
  public LinkedHashSet<E> saveRelatedEntities(UUID entityPartUuid, LinkedHashSet<E> entities) {
    ...
    dbi.withHandle(h -> h.createUpdate("DELETE FROM rel_entitypart_entities WHERE entitypart_uuid = :uuid")
      .bind("uuid", entityPartUuid).execute());
    ...
    if (entities != null) {
      ...
      dbi.useHandle(handle -> {
        ...
        for (Entity entity : entities) {
          preparedBatch.bind("uuid", entityPartUuid)
  ...
  public LinkedHashSet<FileResource> saveRelatedFileResources(UUID entityPartUuid, LinkedHashSet<FileResource> fileResources) {
    ...
    // as we store the whole list new: delete old entries
    dbi.withHandle(h -> h.createUpdate("DELETE FROM rel_entitypart_fileresources WHERE entitypart_uuid = :uuid")
      .bind("uuid", entityPartUuid).execute());
