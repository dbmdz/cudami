...
@SpringBootTest(
    ...
    classes = {CaveRepositoryImpl.class})
@DisplayName("The Cave Repository")
class CaveRepositoryImplTest extends AbstractIdentifiableRepositoryImplTest<CaveRepositoryImpl> {
  @Autowired private IdentifierRepository identifierRepository;
  @Autowired private UrlAliasRepository urlAliasRepository;

  @BeforeEach
  public void beforeEach() {
    repo = new CaveRepositoryImpl(jdbi, cudamiConfig, identifierRepository, urlAliasRepository);
  }
  ...
  void testUpdate() throws RepositoryException, ValidationException {
    ...
    Cave beforeUpdate = createDeepCopy(geolocation);
