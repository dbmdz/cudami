...
@RestController
public class CollectionsAPIController extends AbstractIdentifiablesController<Collection, CudamiCollectionsClient> {

  private static final Logger LOGGER = LoggerFactory.getLogger(CollectionsAPIController.class);

  public CollectionsAPIController(LanguageSortingHelper languageSortingHelper, CudamiClient client) {
  ...
  public ResponseEntity addSubcollection(@PathVariable UUID collectionUuid, @PathVariable UUID subcollectionUuid)
	  throws TechnicalException {
  ...
  public ResponseEntity addSubcollections(@PathVariable UUID collectionUuid,
	  @RequestBody List<Collection> subcollections) throws TechnicalException {
	...
	if (searchField != null) {
	  ...
	  if ("uuid".equals(searchField) || "refId".equals(searchField)) {
		...
		Filtering filtering = Filtering.builder()
			.add(searchField, FilterCriterion.builder().isEquals(searchTerm).build()).build();
		...
		Filtering filtering = Filtering.builder()
			.add(searchField, FilterCriterion.builder().contains(searchTerm).build()).build();
		pageRequest.setFiltering(filtering);
	  }
	} else {
  ...
  public BTResponse<DigitalObject> findDigitalObjects(@PathVariable UUID uuid,
	...
	PageRequest pageRequest = createPageRequest(sort, order, dataLanguage, localeService, offset, limit, searchTerm);
  ...
  @ResponseBody
  public BTResponse<Collection> findSubcollections(@PathVariable UUID uuid,
	  @RequestParam(name = "offset", required = false, defaultValue = "0") int offset,
	  @RequestParam(name = "limit", required = false, defaultValue = "1") int limit,
	  @RequestParam(name = "search", required = false) String searchTerm,
	  @RequestParam(name = "sort", required = false, defaultValue = "label") String sort,
  ...
  @ResponseBody
  public BTResponse<Collection> findTop(@RequestParam(name = "offset", required = false, defaultValue = "0") int offset,
	  @RequestParam(name = "limit", required = false, defaultValue = "1") int limit,
	  @RequestParam(name = "search", required = false) String searchTerm,
	  @RequestParam(name = "sort", required = false, defaultValue = "label") String sort,
	  @RequestParam(name = "order", required = false, defaultValue = "asc") String order,
	  @RequestParam(name = "dataLanguage", required = false) String dataLanguage) throws TechnicalException {
	PageResponse<Collection> pageResponse = service
		.findTopCollections(createPageRequest(sort, order, dataLanguage, localeService, offset, limit, searchTerm));
	return new BTResponse<>(pageResponse);
  }
  ...
  @ResponseBody
  public ResponseEntity removeDigitalObject(@PathVariable UUID collectionUuid, @PathVariable UUID digitalobjectUuid)
	  throws TechnicalException {
  ...
  public ResponseEntity removeSubcollection(@PathVariable UUID collectionUuid, @PathVariable UUID subcollectionUuid)
	  throws TechnicalException {
